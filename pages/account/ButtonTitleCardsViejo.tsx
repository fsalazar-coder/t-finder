  ...
  return (
    ...
    >
      <ButtonsMenuDashboard
        shouldRenderCondition={isDashboard && isData && menuDashboardVisible}
      />
      ...
      <ButtonAddOnTitle
        ...
        shouldRenderCondition={!isDashboard && !isButtonItems && shouldRenderButton}
      />

      <ButtonsEditDelete
        buttons={buttonsEditDelete[buttonType]}
        shouldRenderCondition={!isDashboard && isData && isButtonItems && shouldRenderButton}
      />
    </div>
  )
}
...
export function ButtonsMenuDashboard({ shouldRenderCondition }: MenuDashboardParams) {
  return (
    ...
    <i className='w-fit text-color-text-almost-clear text-2xl flex flex-row justify-center cursor-pointer animate-[appear_0.7s_ease] transition-all'>
      <IconMenuI />
    </i>
  )
};

/**buttons edit dashboard */
export function ButtonsEditOnDashboard({ buttons, shouldRenderCondition }: ButtonEditTitleDashboardParams) {
  return (
    shouldRenderCondition && buttons?.map((button: any) => {
      return (
        <button
        ...
        >
          <i className={
            `${button.action === 'edit' ?
              'text-green-300 lg:text-color-text-clear lg:hover:text-green-500' :
              'text-red-300 lg:text-color-text-clear lg:hover:text-red-500'
...
export function ButtonAddOnTitle({ screenNarrow, nameButtonAddOnTitle, clickAddButton, shouldRenderCondition }: any) {
  return (
    shouldRenderCondition &&
    <button
      className="w-fit h-full flex flex-row items-center lg:hover:cursor-pointer transition-all z-20"
...
/**buttons items cards: edit and/or delete */
export function ButtonsEditDelete({ buttons, shouldRenderCondition }: any) {
  return (
    shouldRenderCondition && buttons?.map((button: any) => {
      return (
        <button
